...
public class Sep_12th_CW1_Unique_Email_Addresses {
...
public int numUniqueEmails(String[] emails) {
      ...
      for (String temp:emails) 
      {
		...
		int indexofat = temp.indexOf('@');
		StringBuilder sb = new StringBuilder();
		String temp1 = temp.substring(0, indexofat);
		
		if(temp1.contains("+")) {
		int indexofplus = temp1.indexOf('+');
		temp1 = temp1.substring(0, indexofplus);
		}
		...
		for (int j = 0; j < len; j++) 
		{
			...
			if(temp1.charAt(j)!='.')
			{
				sb.append(temp1.charAt(j));
			}
						
		}
		
		
		String temp2 = temp.substring(indexofat, temp.length());
		finalset.add(sb.toString().concat(temp2));
